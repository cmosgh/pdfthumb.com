...
- **Node majors move by ticket, onto LTS lines only.** Dependabot proposes only Node patches and minors for the image (`update-types` ignore; a version list like `26.x` misses tags like `26.9.0-alpine`). `.nvmrc` is the one Node version: a Dependabot bump of the Dockerfile's `node` image fails `scripts/check-node-pins.sh` until you update `.nvmrc` on that PR to match.
