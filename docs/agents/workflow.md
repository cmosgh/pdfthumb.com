...
- **Node majors move by ticket, onto LTS lines only.** Dependabot proposes only Node patches and minors for the image (#110). `.nvmrc` is the one Node version: a Dependabot bump of the Dockerfile's `node` image fails `scripts/check-node-pins.sh` until you update `.nvmrc` on that PR to match.
