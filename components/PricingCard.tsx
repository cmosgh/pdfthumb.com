...
const PricingCard: React.FC<PricingCardProps> = ({ tier, billingCycle }) => {
  const cardBaseClasses = "bg-white dark:bg-slate-700 rounded-xl shadow-lg dark:shadow-slate-900/50 p-8 flex flex-col h-full transition-all duration-300";
  const featuredBorderColor = tier.highlightColor ? tier.highlightColor.replace('bg-', 'border-') : 'border-indigo-600';
  const darkFeaturedBorderColor = tier.highlightColor ? tier.highlightColor.replace('bg-', 'dark:border-') : 'dark:border-indigo-500';
  ...
  const displayPrice = isAnnual && tier.priceYearly ? tier.priceYearly : tier.price;
  const displayFrequency = isAnnual && tier.priceFrequencyYearly ? tier.priceFrequencyYearly : tier.priceFrequency;
  const displayDiscountText = isAnnual && tier.annualDiscountText ? tier.annualDiscountText : null;

  const handleCtaClick = () => {
    if (tier.isComingSoon) return;

    if (tier.id === 'enterprise' && tier.ctaText.toLowerCase().includes('contact sales')) {
    ...
    if (tier.priceYearly && tier.id !== 'developer') { // Developer is free, no annual distinction needed in ID
  ...
  return (
    <div className={`${cardBaseClasses} ${featuredClasses}`}>
      ...
      <h3 className={`text-2xl font-bold ${highlightTextColorClass} mb-2`}>{tier.name}</h3>
      <p className="text-slate-500 dark:text-slate-400 mb-1 h-12 min-h-[3rem]">{tier.description}</p>
      ...
      <div className="mb-6">
        <span className={`text-5xl font-extrabold ${highlightTextColorClass}`}>
          {displayPrice.startsWith('$') || displayPrice === "Custom" ? displayPrice : `$${displayPrice}`}
        </span>
        {displayFrequency && <span className="text-slate-500 dark:text-slate-400 text-lg ml-1">{displayFrequency}</span>}
      </div>

      <ul className="space-y-3 mb-8 flex-grow">
        {tier.features.map((feature, index) => (
          <li key={index} className="flex items-start">
            <CheckCircleIcon className={`h-6 w-6 ${highlightTextColorClass.split(' ')[0]} ${highlightTextColorClass.split(' ')[1] || ''} mr-2 flex-shrink-0`} />
      ...
      <Button
        ...
        className={`w-full mt-auto ${tier.isComingSoon ? 'opacity-70 cursor-not-allowed' : ''} 
                    ${tier.isFeatured && tier.highlightColor ? `${tier.highlightColor} hover:${tier.highlightColor?.replace('500','600').replace('600','700').replace('700','800')} 
